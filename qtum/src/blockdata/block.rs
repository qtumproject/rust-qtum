...
use crate::prelude::*;
use crate::{merkle_tree, Network, VarInt, OutPoint};
...
impl Header {
    /// Qtum: returns the header size.
    /// Header size is variable:
    /// Serialized length of fields (version, prev_blockhash, merkle_root, time, bits, nonce etc.)
    pub fn size(&self) -> usize {
        181 + self.signature.len()
    }
...
mod tests {
    ...
    fn block_test() {
        ...
        assert_eq!(real_decode.header.difficulty(network), 0);
    ...
    fn qtum_block_qtum_specific_fields_test() {
        let block = include_bytes!("../../tests/data/testnet_block_1d4dbf11b2f51d129e8d8f8ac33474432dd1fc10d8bcc77567830a0f123e397e.raw").to_vec();
        let decode: Result<Block, _> = deserialize(&block);
        assert!(decode.is_ok());
        let real_decode = decode.unwrap();
