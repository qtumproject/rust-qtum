...
impl NetworkKind {
    /// Returns true if this is real mainnet qtum.
...
impl Network {
    ...
    ///
    /// let network = Network::Qtum;
    /// assert_eq!(network.chain_hash(), ChainHash::QTUM);
    /// ```
    ...
    /// use std::convert::TryFrom;
    ///
    /// assert_eq!(Ok(Network::Qtum), Network::try_from(ChainHash::QTUM));
    /// ```
            ...
            fn visit_str<E: serde::de::Error>(self, s: &str) -> Result<Self::Value, E> {
                Network::from_core_arg(s).map_err(|_| {
                    E::invalid_value(
                        ...
                        &"qtum network encoded as a string (either main, test, signet or regtest)",
                    )
                })
            }
