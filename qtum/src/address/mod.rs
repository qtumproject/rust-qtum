        ...
        match self {
            P2pkh { hash, network } => {
                ...
                prefixed[0] = match network {
                    NetworkKind::Main => PUBKEY_ADDRESS_PREFIX_MAIN_QTUM,
                    NetworkKind::Test => PUBKEY_ADDRESS_PREFIX_TEST_QTUM,
                };
            ...
            P2sh { hash, network } => {
                let mut prefixed = [0; 21];
                prefixed[0] = match network {
                    NetworkKind::Main => SCRIPT_ADDRESS_PREFIX_MAIN_QTUM,
                    NetworkKind::Test => SCRIPT_ADDRESS_PREFIX_TEST_QTUM,
                };
                prefixed[1..].copy_from_slice(&hash[..]);
                base58::encode_check_to_fmt(fmt, &prefixed[..])
            }
...
impl KnownHrp {
    ...
    fn to_hrp(self) -> Hrp {
        match self {
            Self::Mainnet => bech32::hrp::QC,
            Self::Testnets => bech32::hrp::TQ,
            Self::Regtest => bech32::hrp::QCRT,
        }
    }
}
...
mod tests {
    ...
    use crate::network::Network::{Qtum, Testnet};
    ...
    fn test_p2wpkh() {
        ...
        let key = "033bc8c83c52df5712229a2f72206d90192366c36428cb0c12b6af98324d97bfbc"
            ...
            .unwrap();
        let addr = Address::p2wpkh(&key, Qtum);
    ...
    fn test_non_existent_segwit_version() {
        ...
        let addr = Address::from_witness_program(program, Qtum);
        roundtrips(&addr, Qtum);
    }
